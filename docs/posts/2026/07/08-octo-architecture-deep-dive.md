...
> 本文原发于 octo-agent.dev 博客，2026-09 迁到这里。
>
<!-- DESC SEP -->
>
从只有几百行的 agent 循环讲起，拆解 octo-agent 的十个机制——内置 tool 契约、上下文压缩、prompt 缓存、三种寿命的记忆、serve 的坚固性设计、workflow 编排、浏览器自愈、权限管控、回收站、自带电池——以及贯穿始终的一条线：凡是能用机制保证的，就不要指望模型的自觉。
>
<!-- DESC SEP -->
